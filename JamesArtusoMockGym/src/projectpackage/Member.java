...
public class Member implements Comparable<Member>{
    ...
    /**
     Constructor to create a member from raw input. Will convert string dob
     and expiration date to the date data type.
    ...
    /**
     Method to check if the user is 18, will compare
     the given date against today's date.
    ...
    public boolean isEighteen(){
        ...
        return(dob.compareTo(testDate) < 1);
    }
    ...
    /**
     Checks to see if the users date of birth is today.
     Will return false if today is the user's birthday.
     @return boolean, return false if today is birthday, true else wise
     */
    public boolean isBorn(){
        Date todaysDate = new Date();
        return(dob.compareTo(todaysDate) < 0);
    }
